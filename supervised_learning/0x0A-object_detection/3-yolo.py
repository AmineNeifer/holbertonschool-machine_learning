...
class Yolo:
    ...
    def process_outputs(self, outputs, image_size):
        ...
        for i, output in enumerate(outputs):
            ...
            t_xy = output[..., :2]
            t_wh = output[..., 2:4]
            box_confidence = np.expand_dims(
                1/(1 + np.exp(-output[..., 4])), axis=-1)
            box_class_prob = 1/(1 + np.exp(-output[..., 5:]))

            b_wh = anchors * np.exp(t_wh)
            b_wh = b_wh / self.model.inputs[0].shape.as_list()[1:3]
            grid = np.tile(np.indices((g_w, g_h)).T, anchors.shape[0]).reshape(
    ...
    def non_max_suppression(self, boxes, box_classes, box_scores):
        ...
        for c in classes:
            ...
            while ordered_idx.size > 1:
                ...
                a_max = (b[maximum][2] - b[maximum][0]) * \
                    (b[maximum][3] - b[maximum][1])
                a_others = (b[others][:, 2] - b[others][:, 0]) * \
                    (b[others][:, 3] - b[others][:, 1])
        ...
        return np.concatenate(filtered_boxes), \
            np.concatenate(filtered_classes), \
            np.concatenate(filtered_scores)
